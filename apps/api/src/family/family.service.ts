...
import { canManageFamilyPrivacy as canManageFamilyPrivacyByRole } from "./family-privacy-access";
...
export class FamilyService {
  ...
  private isFamily(user: JwtPayload) {
    return (
      user.roles?.some(
        (role) =>
          role.type === RoleType.FAMILIA_RESPONSAVEL ||
          role.level === RoleLevel.FAMILIA,
      ) ?? false
    );
  }
  ...
  private canManageFamilyPrivacy(user: JwtPayload) {
    return canManageFamilyPrivacyByRole(user);
  }

  private retentionUntil(value?: string) {
    const parsed = value
      ? new Date(value)
      : new Date(Date.now() + 730 * 86_400_000);
    if (Number.isNaN(parsed.getTime()) || parsed <= new Date()) {
      throw new BadRequestException("Prazo de retenção familiar inválido");
    }
    if (parsed.getTime() - Date.now() > 730 * 86_400_000) {
      throw new BadRequestException(
        "Prazo de retenção familiar não pode exceder 2 anos",
      );
    }
    return parsed;
  }
  ...
  private unitScopeIds(user: JwtPayload): string[] | null {
    if (this.hasLevel(user, RoleLevel.DEVELOPER, RoleLevel.MANTENEDORA))
      return null;
  ...
  private async assertUnitScope(user: JwtPayload, unitId: string) {
    ...
    if (!unit)
      throw new ForbiddenException("Unidade fora do escopo da mantenedora");
  ...
  private async scopedChildWhere(
    user: JwtPayload,
    query: FamilyChildrenQueryDto,
    ...
    if (this.isFamily(user)) {
      where.guardianLinks = {
        some: { userId: user.sub, revokedAt: null, canViewTimeline: true },
      };
    } else {
      const allowedUnitIds = this.unitScopeIds(user);
      if (query.unitId) {
    ...
    if (query.classroomId) {
      ...
      if (!classroom)
        throw new NotFoundException("Turma não encontrada no escopo");
      ...
      where.enrollments = {
        some: { classroomId: classroom.id, status: "ATIVA" },
      };
    }
  ...
  private async activeGuardian(
    user: JwtPayload,
    childId: string,
    permission: "timeline" | "development" = "timeline",
  ) {
    ...
    if (!link)
      throw new ForbiddenException(
        "Criança não vinculada ao responsável ou permissão revogada",
      );
    return link;
  }

  private async assertChildScope(
    user: JwtPayload,
    childId: string,
    permission: "timeline" | "development" = "timeline",
  ) {
    ...
    if (this.isFamily(user)) {
      ...
      const allowedUnitIds = this.unitScopeIds(user);
      if (allowedUnitIds && !allowedUnitIds.includes(child.unitId))
  ...
  async listChildren(user: JwtPayload, query: FamilyChildrenQueryDto) {
    ...
    const items = children.map((child) => {
      ...
      const link =
        "guardianLinks" in child ? child.guardianLinks[0] : undefined;
      return {
        id: child.id,
  ...
  async listGuardianCandidates(
    user: JwtPayload,
    query: FamilyGuardianCandidatesQueryDto,
  ) {
    if (
      !this.hasLevel(
        user,
      ...
      throw new ForbiddenException(
        "Perfil sem permissão para listar responsáveis",
      );
    }
    if (!user.mantenedoraId)
      throw new ForbiddenException("Escopo de mantenedora ausente");
    ...
    const where: Prisma.UserWhereInput = {
      ...
      roles: {
        some: {
          isActive: true,
          role: { isActive: true, type: RoleType.FAMILIA_RESPONSAVEL },
        },
      },
      ...(search
        ? {
  ...
  async listGuardians(childId: string, user: JwtPayload) {
    ...
    if (this.isFamily(user))
      throw new ForbiddenException(
        "Somente equipe autorizada pode listar responsáveis",
      );
      ...
      audit: auditEvents
        .filter((event) => {
          ...
          return (
            ...
            (changes as Record<string, unknown>).guardianUserId === link.userId
          );
        })
        .map((event) => ({
          id: event.id,
  ...
  async linkGuardian(
    ...
    if (!this.canManageFamilyPrivacy(user)) {
      throw new ForbiddenException(
        "Somente administrativo, direção ou mantenedora podem gerenciar vínculos familiares",
      );
    }
    if (dto.legalBasis && dto.legalBasis !== Onda1LegalBasis.CONSENT) {
      throw new BadRequestException(
        "Vínculo familiar exige consentimento como base legal",
      );
    }
    const legalBasis = dto.legalBasis ?? Onda1LegalBasis.CONSENT;
    const consentPolicyVersion =
      dto.consentPolicyVersion?.trim() || "family-link-v1";
    const retentionUntil = this.retentionUntil(dto.retentionUntil);
    ...
    if (!guardian)
      throw new NotFoundException(
        "Usuário não é um responsável familiar ativo da mantenedora",
      );
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await tx.consentGrant.create({
        data: {
          ...
          unitId: child.unitId,
          childId: child.id,
          responsibleUserId: guardian.id,
          purpose: Onda1ConsentPurpose.VINCULO_ACESSO,
          decision: Onda1ConsentDecision.CONCEDIDO,
          legalBasis,
          policyVersion: consentPolicyVersion,
          textPresented:
            "Vínculo familiar e permissões de acesso explicitamente autorizados.",
          origin: "FAMILY_GUARDIAN_LINK",
          validUntil: retentionUntil,
          decidedAt: now,
        },
      });
      await tx.auditLog.create({
        data: {
          ...
          description: existing
            ? "Vínculo familiar atualizado"
            : "Vínculo familiar criado",
          changes: {
            guardianUserId: guardian.id,
            relationship: link.relationship,
            isPrimary: link.isPrimary,
            canViewTimeline: link.canViewTimeline,
            canViewDevelopment: link.canViewDevelopment,
  ...
  async revokeGuardian(
    ...
    dto: RevokeGuardianDto,
    user: JwtPayload,
  ) {
    if (!this.canManageFamilyPrivacy(user)) {
      throw new ForbiddenException(
        "Somente administrativo, direção ou mantenedora podem revogar vínculos familiares",
      );
    }
    const reason = dto.reason.trim();
    if (reason.length < 3)
      throw new BadRequestException("Motivo de revogação obrigatório");
    const child = await this.assertChildScope(user, childId);
    const now = new Date();
    return this.prisma.$transaction(async (tx) => {
      const current = await tx.childGuardian.findFirst({
        where: {
          childId,
          userId: guardianUserId,
          child: { mantenedoraId: user.mantenedoraId },
          revokedAt: null,
        },
        select: {
          legalBasis: true,
      ...
      if (result.count > 0) {
        await tx.consentGrant.create({
          data: {
            ...
            unitId: child.unitId,
            childId: child.id,
            responsibleUserId: guardianUserId,
            purpose: Onda1ConsentPurpose.VINCULO_ACESSO,
            decision: Onda1ConsentDecision.REVOGADO,
            legalBasis: current?.legalBasis ?? Onda1LegalBasis.CONSENT,
            policyVersion: current?.consentPolicyVersion ?? "family-link-v1",
            textPresented: "Vínculo familiar revogado.",
            origin: "FAMILY_GUARDIAN_REVOKE",
            validUntil: current?.retentionUntil ?? now,
            revokedAt: now,
            decidedAt: now,
          },
        });
  ...
  async timeline(
    ...
    const developmentObservationSelect = {
      id: true,
      category: true,
      date: true,
    ...
    } satisfies Prisma.DevelopmentObservationSelect;
    type TimelineObservation = Prisma.DevelopmentObservationGetPayload<{
      select: typeof developmentObservationSelect;
    }>;
    ...
    const [diaryEvents, posts, observations, messages] = await Promise.all([
      ...
      this.prisma.studentPostPerformance.findMany({
        where: {
          childId,
          post: { status: "PUBLICADO", createdAt: { gte: from, lte: to } },
        },
        select: {
          id: true,
          performance: true,
      ...
      familyCanViewDevelopment
        ? this.prisma.developmentObservation.findMany({
            ...
            select: developmentObservationSelect,
            orderBy: { date: "desc" },
          })
        : Promise.resolve([] as TimelineObservation[]),
    ...
    const items = [
      ...
      ...observations.map((observation) => ({
        ...
        title: `Observação · ${observation.category}`,
        body:
  ...
  async listMessages(user: JwtPayload, query: FamilyQueryDto) {
    ...
    const where = this.isFamily(user)
      ? {
          childId,
          OR: [{ senderUserId: user.sub }, { recipientUserId: user.sub }],
          child: { mantenedoraId: user.mantenedoraId },
        }
      : {
          childId,
  ...
  async sendMessage(
    ...
    if (this.isFamily(user)) {
      ...
      if (!recipientUserId)
        throw new NotFoundException(
          "Nenhum responsável ativo encontrado para receber a mensagem",
        );
    }
    ...
    await this.evidenceService?.syncSafely("FAMILY_COMMUNICATION", () =>
      this.evidenceService!.syncFamilyCommunication(message),
    );
    return message;
  }

  async markMessageRead(id: string, user: JwtPayload) {
    const message = await this.prisma.familyCommunication.findFirst({
      where: { id, child: { mantenedoraId: user.mantenedoraId } },
    });
    if (!message) throw new NotFoundException("Mensagem não encontrada");
    if (message.recipientUserId !== user.sub)
      throw new ForbiddenException(
        "Somente o destinatário pode marcar a mensagem como lida",
      );
    return this.prisma.familyCommunication.update({
      where: { id },
      data: { status: FamilyCommunicationStatus.LIDA, readAt: new Date() },
    });
  }
}
