...
export class Onda1FamilyService {
  ...
  private async assertFlag(user: JwtPayload) {
    await this.access.assertFlagEnabled(
      user,
      ONDA1_FEATURE_FLAGS.familyCircleV1,
    );
  }
  ...
  private async childInScope(childId: string, user: JwtPayload) {
    ...
    if (!child)
      throw new NotFoundException(
        "Criança não encontrada no escopo da mantenedora.",
      );
    if (this.access.isNetworkScoped(user) || user.unitId === child.unitId)
      return child;
    if (this.access.isCentralScoped(user)) {
      const centralRole = user.roles?.find(
        (role) => role.level === RoleLevel.STAFF_CENTRAL,
      );
      const scopes = Array.isArray(centralRole?.unitScopes)
        ? centralRole.unitScopes
        : [];
      if (scopes.length === 0 || scopes.includes(child.unitId)) return child;
    }
    if (this.access.isTeacher(user)) {
      const classroomIds = child.enrollments.map(
        (enrollment) => enrollment.classroomId,
      );
      const assigned = await this.prisma.classroomTeacher.findFirst({
        where: {
          teacherId: user.sub,
  ...
  private async conversationInScope(conversationId: string, user: JwtPayload) {
    const conversation = await this.prisma.familyConversation.findFirst({
      where: { id: conversationId, mantenedoraId: user.mantenedoraId },
    });
    if (!conversation) throw new NotFoundException("Conversa não encontrada.");
    await this.childInScope(conversation.childId, user);
    if (user.roles?.some((role) => role.level === RoleLevel.FAMILIA)) {
      const linked = await this.access.canViewFamilyChild(
        user,
        conversation.childId,
      );
      if (!linked)
        throw new ForbiddenException(
          "Família sem vínculo ativo com a criança.",
        );
    }
    return conversation;
  }
  ...
  private async activeConsent(
    ...
    if (!consentGrantId)
      throw new ForbiddenException(
        "Publicação familiar exige consentimento explícito.",
      );
    const now = new Date();
    ...
    if (!consent)
      throw new ForbiddenException(
        "Consentimento inexistente, revogado ou expirado.",
      );
    return consent;
  }
  ...
  async feed(childId: string, query: FamilyFeedQueryDto, user: JwtPayload) {
    ...
    const [publicationsPage, conversations, contributions] = await Promise.all([
      this.prisma.publicationRecord.findMany({
        where: publicationWhere,
        orderBy: [{ publishedAt: "desc" }, { id: "desc" }],
        take: limit + 1,
        ...(query.cursor ? { skip: 1, cursor: { id: query.cursor } } : {}),
      }),
      this.prisma.familyConversation.findMany({
        where: {
          mantenedoraId: user.mantenedoraId,
          childId,
          ...(user.roles?.some((role) => role.level === RoleLevel.FAMILIA)
            ? {}
            : {}),
        },
      ...
      this.prisma.familyContribution.findMany({
        where: {
          mantenedoraId: user.mantenedoraId,
          childId,
          status: Onda1ReviewTaskStatus.APPROVED,
          visibility: "FAMILIA_AUTORIZADA",
        },
    ...
    const publications = hasMore
      ? publicationsPage.slice(0, limit)
      : publicationsPage;
    return {
      child: {
        id: child.id,
      ...
      pageInfo: {
        hasMore,
        nextCursor: hasMore
          ? (publications[publications.length - 1]?.id ?? null)
          : null,
      },
      governance: {
        consentGated: true,
  ...
  async listConversations(childId: string, user: JwtPayload) {
    ...
    return this.prisma.familyConversation.findMany({
      where: { mantenedoraId: user.mantenedoraId, childId },
      orderBy: { updatedAt: "desc" },
      take: 100,
      include: { messages: { orderBy: { createdAt: "desc" }, take: 1 } },
    });
  ...
  async createConversation(dto: CreateFamilyConversationDto, user: JwtPayload) {
    ...
    const conversation = await this.prisma.$transaction(async (tx) => {
      const created = await tx.familyConversation.create({
        data: {
          mantenedoraId: user.mantenedoraId,
          unitId: child.unitId,
          childId: child.id,
      ...
      await this.outbox(
        tx,
        user,
        "FamilyConversationCreated",
        "FamilyConversation",
        created.id,
        { conversationId: created.id, childId: child.id },
      );
      return created;
    });
    await this.audit.log({
      action: "CREATE",
      entity: "FAMILY_CONVERSATION",
  ...
  async updateConversation(
    ...
    dto: UpdateFamilyConversationDto,
    user: JwtPayload,
  ) {
    await this.assertFlag(user);
    ...
    const updated = await this.prisma.familyConversation.update({
      where: { id: conversationId },
      data: {
        status: dto.status,
        closedBy:
          dto.status === Onda1ConversationStatus.ENCERRADA ? user.sub : null,
        closedAt:
          dto.status === Onda1ConversationStatus.ENCERRADA ? new Date() : null,
      },
    });
    await this.audit.log({
      action: "UPDATE",
  ...
  async listMessages(
    conversationId: string,
    query: FamilyFeedQueryDto,
    user: JwtPayload,
  ) {
    await this.assertFlag(user);
    await this.conversationInScope(conversationId, user);
    const limit = Math.min(Math.max(query.limit ?? 30, 1), 100);
    const page = await this.prisma.familyMessageV2.findMany({
  ...
  async createMessage(
    ...
    if (conversation.status !== Onda1ConversationStatus.ABERTA)
      throw new BadRequestException("A conversa está encerrada.");
    if (dto.clientMutationId) {
      const existing = await this.prisma.familyMessageV2.findFirst({
        where: { conversationId, clientMutationId: dto.clientMutationId },
      });
      if (existing) return existing;
    }
    const message = await this.prisma.$transaction(async (tx) => {
      const created = await tx.familyMessageV2.create({
        data: {
          conversationId,
          ...
          status: Onda1MessageStatus.ENVIADA,
        },
      });
      await this.outbox(
        tx,
        user,
        "FamilyMessageCreated",
        "FamilyMessageV2",
        created.id,
        { conversationId, messageId: created.id },
      );
      return created;
    });
    await this.audit.log({
  ...
  async listContributions(childId: string, user: JwtPayload) {
    await this.assertFlag(user);
    await this.childInScope(childId, user);
    const isFamily = user.roles?.some(
      (role) => role.level === RoleLevel.FAMILIA,
    );
    return this.prisma.familyContribution.findMany({
      where: {
        mantenedoraId: user.mantenedoraId,
        childId,
        ...(isFamily
  ...
  async createContribution(dto: CreateFamilyContributionDto, user: JwtPayload) {
    ...
    const child = await this.childInScope(dto.childId, user);
    if (dto.goalId) {
      const goal = await this.prisma.childGoal.findFirst({
        where: {
          id: dto.goalId,
          childId: child.id,
          mantenedoraId: user.mantenedoraId,
      ...
      if (!goal)
        throw new NotFoundException(
          "Objetivo não encontrado para a contribuição.",
        );
    }
    if (dto.evidenceId) {
      const evidence = await this.prisma.childEvidence.findFirst({
        where: {
          id: dto.evidenceId,
          childId: child.id,
          mantenedoraId: user.mantenedoraId,
      ...
      if (!evidence)
        throw new NotFoundException(
          "Evidência não encontrada para a contribuição.",
        );
    }
    const contribution = await this.prisma.$transaction(async (tx) => {
      const created = await tx.familyContribution.create({
        data: {
          mantenedoraId: user.mantenedoraId,
          unitId: child.unitId,
          childId: child.id,
          ...
          structuredData: dto.structuredData as
      ...
      await this.outbox(
        tx,
        user,
        "FamilyContributionCreated",
        "FamilyContribution",
        created.id,
        { contributionId: created.id, childId: child.id },
      );
      return created;
    });
    await this.audit.log({
      action: "CREATE",
      entity: "FAMILY_CONTRIBUTION",
  ...
  async createConsent(dto: CreateConsentGrantDto, user: JwtPayload) {
    ...
    const consent = await this.prisma.$transaction(async (tx) => {
      const created = await tx.consentGrant.create({
        data: {
          ...
          textPresented: dto.textPresented,
          origin: dto.origin.trim(),
          proof: dto.proof as Prisma.InputJsonValue | undefined,
    ...
    await this.audit.log({
      ...
      mantenedoraId: user.mantenedoraId,
      unitId: child.unitId,
      changes: { purpose: consent.purpose, decision: consent.decision },
    });
    return consent;
  }
  ...
  async listConsents(childId: string, user: JwtPayload) {
    ...
    return this.prisma.consentGrant.findMany({
      where: {
        mantenedoraId: user.mantenedoraId,
        childId,
        ...(isFamily ? { responsibleUserId: user.sub } : {}),
      },
  ...
  async createPublication(dto: CreatePublicationDto, user: JwtPayload) {
    ...
    const child = await this.childInScope(dto.childId, user);
    const consent =
      dto.audienceType === "FAMILIA"
        ? await this.activeConsent(child.id, dto.consentGrantId, user)
        : undefined;
    const publication = await this.prisma.publicationRecord.create({
      data: {
        mantenedoraId: user.mantenedoraId,
        unitId: child.unitId,
        childId: child.id,
        ...
        status: Onda1PublicationStatus.RASCUNHO,
        createdBy: user.sub,
      },
    });
    await this.audit.log({
      action: "CREATE",
      entity: "PUBLICATION_RECORD",
  ...
  async publishPublication(publicationId: string, user: JwtPayload) {
    await this.assertFlag(user);
    this.access.assertCapability(user, ONDA1_CAPABILITIES.familyPublish);
    const publication = await this.prisma.publicationRecord.findFirst({
      where: { id: publicationId, mantenedoraId: user.mantenedoraId },
    });
    if (!publication) throw new NotFoundException("Publicação não encontrada.");
    await this.childInScope(publication.childId, user);
    if (publication.audienceType === "FAMILIA")
      await this.activeConsent(
        publication.childId,
        publication.consentGrantId ?? undefined,
        user,
      );
    if (publication.status === Onda1PublicationStatus.PUBLICADA)
      return publication;
    const updated = await this.prisma.$transaction(async (tx) => {
      const result = await tx.publicationRecord.update({
  ...
  async acknowledge(dto: CreateAcknowledgmentDto, user: JwtPayload) {
    ...
    const acknowledgment = await this.prisma.acknowledgment.upsert({
      where: {
        recordType_recordId_userId_kind: {
  ...
  async upsertCommunicationPreference(
    dto: UpsertCommunicationPreferenceDto,
    user: JwtPayload,
  ) {
    await this.assertFlag(user);
    this.access.assertCapability(user, ONDA1_CAPABILITIES.familyMessage);
    const preference = await this.prisma.communicationPreference.upsert({
      ...
      create: {
        ...
        importantAlerts: dto.importantAlerts ?? true,
      },
      update: {
        locale: dto.locale,
        channels: dto.channels as Prisma.InputJsonValue | undefined,
