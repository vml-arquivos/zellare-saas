...
const inputClass = 'mt-1 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-800';
const buttonClass = 'inline-flex items-center justify-center gap-2 rounded-lg bg-indigo-600 px-3 py-2 text-sm font-medium text-white hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-50';
    ...
    <PageShell title="Financeiro e gestão de pessoas" subtitle="Folha, ponto, pagamentos, compras e estoque conectados às tabelas reais do Zelare.">
      <div className="space-y-6">
        ...
        <section className="grid gap-4 md:grid-cols-4">
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><CalendarDays className="h-5 w-5 text-indigo-600" /><p className="mt-3 text-2xl font-semibold text-slate-900">{selectedPeriod ? formatMonth(selectedPeriod.referenceMonth) : '—'}</p><p className="text-xs text-slate-500">{selectedPeriod ? STATUS_LABELS[selectedPeriod.status] : 'Sem competência'}</p></div>
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><Users className="h-5 w-5 text-indigo-600" /><p className="mt-3 text-2xl font-semibold text-slate-900">{employees.length}</p><p className="text-xs text-slate-500">Funcionários no escopo</p></div>
        ...
        <div className="flex flex-wrap gap-2 rounded-2xl border border-slate-200 bg-white p-2 shadow-sm">
          {([['controle', 'Ponto e competências', Clock3], ['folha', 'Folha', FileCheck2], ['contas', 'Contas a pagar', Receipt], ['estoque', 'Compras e estoque', Boxes]] as const).map(([tab, label, Icon]) => (
            <button key={tab} type="button" onClick={() => setActiveTab(tab)} className={`inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-medium ${activeTab === tab ? 'bg-indigo-600 text-white' : 'text-slate-600 hover:bg-slate-100'}`}><Icon className="h-4 w-4" />{label}</button>
          ))}
        ...
        {activeTab === 'controle' && <>
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><h2 className="text-lg font-semibold text-slate-900">Competências financeiras</h2><p className="mt-1 text-sm text-slate-500">O fechamento é controlado por estado e não sobrescreve competências fechadas.</p></div><div className="flex flex-wrap items-end gap-2"><label className="text-xs font-medium text-slate-600">Nova competência<input type="month" value={newMonth} onChange={(event) => setNewMonth(event.target.value)} className="mt-1 block rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-800" /></label><button type="button" onClick={() => void handleCreatePeriod()} disabled={saving || !newMonth} className={buttonClass}><Plus className="h-4 w-4" />Abrir competência</button></div></div>
            <div className="mt-5 overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="border-b border-slate-200 text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-3 py-3">Competência</th><th className="px-3 py-3">Status</th><th className="px-3 py-3">Fechada em</th><th className="px-3 py-3 text-right">Ação</th></tr></thead><tbody className="divide-y divide-slate-100">{periods.map((period) => <tr key={period.id} className={period.id === selectedPeriod?.id ? 'bg-indigo-50/60' : ''}><td className="px-3 py-3 font-medium">{formatMonth(period.referenceMonth)}</td><td className="px-3 py-3"><button type="button" onClick={() => setSelectedPeriodId(period.id)} className={`rounded-full border px-2.5 py-1 text-xs ${STATUS_COLORS[period.status]}`}>{STATUS_LABELS[period.status]}</button></td><td className="px-3 py-3 text-slate-500">{formatDate(period.closedAt)}</td><td className="px-3 py-3 text-right">{period.status === 'ABERTA' && <button type="button" onClick={() => void run(async () => { await updateFinancialPeriodStatus(period.id, 'EM_CONFERENCIA'); }, 'Competência enviada para conferência.')} className="rounded-lg border border-amber-300 px-2.5 py-1.5 text-xs text-amber-800"><CheckCircle2 className="mr-1 inline h-3.5 w-3.5" />Enviar</button>}{period.status === 'FECHADA' && <span className="text-xs text-slate-500"><LockKeyhole className="mr-1 inline h-3.5 w-3.5" />Protegida</span>}</td></tr>)}{!loading && periods.length === 0 && <tr><td colSpan={4} className="px-3 py-10 text-center text-slate-500">Nenhuma competência real encontrada.</td></tr>}</tbody></table></div>
          </section>
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><div><h2 className="text-lg font-semibold text-slate-900">Registrar ponto</h2><p className="mt-1 text-sm text-slate-500">O registro nasce como rascunho e precisa ser enviado para conferência.</p></div><Clock3 className="h-5 w-5 text-indigo-600" /></div><form className="mt-4 grid gap-3 md:grid-cols-3" onSubmit={(event) => { event.preventDefault(); void handleCreateTimeEntry(); }}><label className="text-xs font-medium text-slate-600">Funcionário<select required value={timeForm.employeeId} onChange={(event) => setTimeForm({ ...timeForm, employeeId: event.target.value })} className={inputClass}><option value="">Selecione</option>{employees.map((employee) => <option key={employee.id} value={employee.id}>{employee.firstName} {employee.lastName} · {employee.employeeCode}</option>)}</select></label><label className="text-xs font-medium text-slate-600">Data<input required type="date" value={timeForm.workDate} onChange={(event) => setTimeForm({ ...timeForm, workDate: event.target.value })} className={inputClass} /></label><label className="text-xs font-medium text-slate-600">Intervalo (minutos)<input min="0" type="number" value={timeForm.breakMinutes} onChange={(event) => setTimeForm({ ...timeForm, breakMinutes: event.target.value })} className={inputClass} /></label><label className="text-xs font-medium text-slate-600">Entrada<input type="time" value={timeForm.clockIn} onChange={(event) => setTimeForm({ ...timeForm, clockIn: event.target.value })} className={inputClass} /></label><label className="text-xs font-medium text-slate-600">Saída<input type="time" value={timeForm.clockOut} onChange={(event) => setTimeForm({ ...timeForm, clockOut: event.target.value })} className={inputClass} /></label><label className="text-xs font-medium text-slate-600">Observação<input maxLength={240} value={timeForm.notes} onChange={(event) => setTimeForm({ ...timeForm, notes: event.target.value })} className={inputClass} /></label><div className="md:col-span-3"><button type="submit" disabled={saving || !selectedPeriod} className={buttonClass}><Plus className="h-4 w-4" />Registrar ponto</button></div></form></section>
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><div><h2 className="text-lg font-semibold text-slate-900">Registros de ponto</h2><p className="mt-1 text-sm text-slate-500">Dados filtrados pela competência selecionada.</p></div><Clock3 className="h-5 w-5 text-indigo-600" /></div><div className="mt-5 overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="border-b border-slate-200 text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-3 py-3">Data</th><th className="px-3 py-3">Funcionário</th><th className="px-3 py-3">Entrada</th><th className="px-3 py-3">Saída</th><th className="px-3 py-3">Status</th></tr></thead><tbody className="divide-y divide-slate-100">{timeEntries.slice(0, 100).map((entry) => { const employee = employees.find((item) => item.id === entry.employeeId); return <tr key={entry.id}><td className="px-3 py-3">{formatDate(entry.workDate)}</td><td className="px-3 py-3 font-medium">{employee ? `${employee.firstName} ${employee.lastName}` : entry.employeeId}</td><td className="px-3 py-3 text-slate-500">{formatDate(entry.clockIn)}</td><td className="px-3 py-3 text-slate-500">{formatDate(entry.clockOut)}</td><td className="px-3 py-3 text-slate-500">{entry.status}{entry.status === 'RASCUNHO' && <button type="button" onClick={() => void run(async () => { await submitTimeEntry(entry.id); }, 'Registro enviado para conferência.')} className="ml-2 rounded-lg border border-indigo-200 px-2 py-1 text-xs text-indigo-700">Enviar</button>}</td></tr>; })}{!loading && timeEntries.length === 0 && <tr><td colSpan={5} className="px-3 py-10 text-center text-slate-500">Nenhum ponto na competência selecionada.</td></tr>}</tbody></table></div></section>
        </>}

        {activeTab === 'folha' && <section className="space-y-5"><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><h2 className="text-lg font-semibold">Folha de pagamento</h2><p className="mt-1 text-sm text-slate-500">Cálculo auditável com snapshot dos funcionários ativos e transições de aprovação.</p></div><button type="button" onClick={() => void handlePayroll()} disabled={!selectedPeriod || saving} className={buttonClass}><FileCheck2 className="h-4 w-4" />Calcular competência selecionada</button></div><div className="mt-5 grid gap-4 md:grid-cols-4"><div className="rounded-xl bg-slate-50 p-4"><p className="text-xs text-slate-500">Status</p><p className="mt-1 font-semibold">{currentPayroll ? PAYROLL_LABELS[currentPayroll.status] : 'Não calculada'}</p></div><div className="rounded-xl bg-slate-50 p-4"><p className="text-xs text-slate-500">Bruto</p><p className="mt-1 font-semibold">{money(currentPayroll?.totalGross)}</p></div><div className="rounded-xl bg-slate-50 p-4"><p className="text-xs text-slate-500">Descontos</p><p className="mt-1 font-semibold">{money(currentPayroll?.totalDeductions)}</p></div><div className="rounded-xl bg-slate-50 p-4"><p className="text-xs text-slate-500">Líquido</p><p className="mt-1 font-semibold">{money(currentPayroll?.totalNet)}</p></div></div></div><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h3 className="font-semibold">Histórico de folhas</h3><div className="mt-4 overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="border-b border-slate-200 text-xs uppercase text-slate-500"><tr><th className="px-3 py-3">Competência</th><th className="px-3 py-3">Status</th><th className="px-3 py-3">Bruto</th><th className="px-3 py-3">Líquido</th><th className="px-3 py-3">Ação</th></tr></thead><tbody className="divide-y divide-slate-100">{payrolls.map((payroll) => { const period = periods.find((item) => item.id === payroll.periodId); const next = nextPayrollStatus(payroll.status); return <tr key={payroll.id}><td className="px-3 py-3">{period ? formatMonth(period.referenceMonth) : payroll.periodId}</td><td className="px-3 py-3">{PAYROLL_LABELS[payroll.status]}</td><td className="px-3 py-3">{money(payroll.totalGross)}</td><td className="px-3 py-3">{money(payroll.totalNet)}</td><td className="px-3 py-3">{next && <button type="button" onClick={() => void run(async () => { await updatePayrollStatus(payroll.id, next); }, `Folha movida para ${PAYROLL_LABELS[next]}.`)} className="rounded-lg border border-indigo-200 px-2.5 py-1.5 text-xs text-indigo-700">Avançar para {PAYROLL_LABELS[next]}</button>}</td></tr>; })}{!payrolls.length && <tr><td colSpan={5} className="px-3 py-10 text-center text-slate-500">Nenhuma folha calculada.</td></tr>}</tbody></table></div></div></section>}

        {activeTab === 'contas' && <section className="space-y-5"><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center gap-2"><Receipt className="h-5 w-5 text-indigo-600" /><div><h2 className="text-lg font-semibold">Nova conta a pagar</h2><p className="text-sm text-slate-500">Crie o título e faça o fluxo de aprovação até a conciliação.</p></div></div><form className="mt-5 grid gap-3 md:grid-cols-3" onSubmit={(event) => { event.preventDefault(); void run(async () => { await createPayable({ beneficiary: payableForm.beneficiary, description: payableForm.description, category: payableForm.category, sourceType: 'MANUAL', dueDate: new Date(`${payableForm.dueDate}T12:00:00`).toISOString(), amount: Number(payableForm.amount), documentRef: payableForm.documentRef }); setPayableForm({ beneficiary: '', description: '', category: 'Operacional', dueDate: '', amount: '', documentRef: '' }); }, 'Conta a pagar criada como rascunho.'); }}><label className="text-xs font-medium">Beneficiário<input required value={payableForm.beneficiary} onChange={(event) => setPayableForm({ ...payableForm, beneficiary: event.target.value })} className={inputClass} /></label><label className="text-xs font-medium">Descrição<input required value={payableForm.description} onChange={(event) => setPayableForm({ ...payableForm, description: event.target.value })} className={inputClass} /></label><label className="text-xs font-medium">Categoria<input required value={payableForm.category} onChange={(event) => setPayableForm({ ...payableForm, category: event.target.value })} className={inputClass} /></label><label className="text-xs font-medium">Vencimento<input required type="date" value={payableForm.dueDate} onChange={(event) => setPayableForm({ ...payableForm, dueDate: event.target.value })} className={inputClass} /></label><label className="text-xs font-medium">Valor<input required min="0" step="0.01" type="number" value={payableForm.amount} onChange={(event) => setPayableForm({ ...payableForm, amount: event.target.value })} className={inputClass} /></label><label className="text-xs font-medium">Documento<input value={payableForm.documentRef} onChange={(event) => setPayableForm({ ...payableForm, documentRef: event.target.value })} className={inputClass} /></label><div className="md:col-span-3"><button className={buttonClass} disabled={saving}><Plus className="h-4 w-4" />Criar conta</button></div></form></div><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h3 className="font-semibold">Contas reais no escopo</h3><div className="mt-4 overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="border-b border-slate-200 text-xs uppercase text-slate-500"><tr><th className="px-3 py-3">Beneficiário</th><th className="px-3 py-3">Vencimento</th><th className="px-3 py-3">Valor</th><th className="px-3 py-3">Status</th><th className="px-3 py-3">Fluxo</th></tr></thead><tbody className="divide-y divide-slate-100">{payables.map((payable) => { const next = nextPayableStatus(payable.status); return <tr key={payable.id}><td className="px-3 py-3 font-medium">{payable.beneficiary}<span className="block text-xs font-normal text-slate-500">{payable.description}</span></td><td className="px-3 py-3">{formatDate(payable.dueDate)}</td><td className="px-3 py-3">{money(payable.amount)}</td><td className="px-3 py-3">{PAYABLE_LABELS[payable.status]}</td><td className="px-3 py-3">{next && <div className="flex flex-wrap gap-2">{next === 'PAGA' && <input placeholder="Referência" value={paymentRefs[payable.id] || ''} onChange={(event) => setPaymentRefs({ ...paymentRefs, [payable.id]: event.target.value })} className="w-28 rounded border border-slate-300 px-2 py-1 text-xs" />}<button type="button" onClick={() => void run(async () => { await updatePayableStatus(payable.id, next, paymentRefs[payable.id]); }, `Conta movida para ${PAYABLE_LABELS[next]}.`)} className="rounded-lg border border-indigo-200 px-2.5 py-1.5 text-xs text-indigo-700">{PAYABLE_LABELS[next]}</button></div>}</td></tr>; })}{!payables.length && <tr><td colSpan={5} className="px-3 py-10 text-center text-slate-500">Nenhuma conta encontrada.</td></tr>}</tbody></table></div></div></section>}

        {activeTab === 'estoque' && <section className="space-y-5"><div className="grid gap-5 xl:grid-cols-2"><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center gap-2"><Boxes className="h-5 w-5 text-indigo-600" /><h2 className="text-lg font-semibold">Cadastrar item de estoque</h2></div><form className="mt-4 grid gap-3 md:grid-cols-2" onSubmit={(event) => { event.preventDefault(); void run(async () => { await createStockItem({ unitId: stockForm.unitId, code: stockForm.code, name: stockForm.name, minimumQuantity: Number(stockForm.minimumQuantity), location: stockForm.location }); setStockForm({ ...stockForm, code: '', name: '', minimumQuantity: '0', location: '' }); }, 'Item de estoque criado.'); }}><label className="text-xs font-medium">Unidade ID<input required value={stockForm.unitId} onChange={(event) => setStockForm({ ...stockForm, unitId: event.target.value })} className={inputClass} /></label><label className="text-xs font-medium">Código<input required value={stockForm.code} onChange={(event) => setStockForm({ ...stockForm, code: event.target.value })} className={inputClass} /></label><label className="text-xs font-medium">Nome<input required value={stockForm.name} onChange={(event) => setStockForm({ ...stockForm, name: event.target.value })} className={inputClass} /></label><label className="text-xs font-medium">Mínimo<input type="number" min="0" value={stockForm.minimumQuantity} onChange={(event) => setStockForm({ ...stockForm, minimumQuantity: event.target.value })} className={inputClass} /></label><label className="text-xs font-medium md:col-span-2">Localização<input value={stockForm.location} onChange={(event) => setStockForm({ ...stockForm, location: event.target.value })} className={inputClass} /></label><button className={buttonClass} disabled={saving}><Plus className="h-4 w-4" />Cadastrar item</button></form></div><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center gap-2"><ShoppingCart className="h-5 w-5 text-indigo-600" /><h2 className="text-lg font-semibold">Movimentar estoque</h2></div><form className="mt-4 grid gap-3 md:grid-cols-2" onSubmit={(event) => { event.preventDefault(); void run(async () => { await createStockMovement({ unitId: movementForm.unitId, stockItemId: movementForm.stockItemId, movementType: movementForm.movementType, quantity: Number(movementForm.quantity), unitCost: movementForm.unitCost ? Number(movementForm.unitCost) : undefined, sourceType: movementForm.sourceType, reason: movementForm.reason }); }, 'Movimento de estoque registrado.'); }}><label className="text-xs font-medium">Unidade ID<input required value={movementForm.unitId} onChange={(event) => setMovementForm({ ...movementForm, unitId: event.target.value })} className={inputClass} /></label><label className="text-xs font-medium">Item<select required value={movementForm.stockItemId} onChange={(event) => setMovementForm({ ...movementForm, stockItemId: event.target.value })} className={inputClass}><option value="">Selecione</option>{stockItems.map((item) => <option key={item.id} value={item.id}>{item.code} — {item.name}</option>)}</select></label><label className="text-xs font-medium">Tipo<select value={movementForm.movementType} onChange={(event) => setMovementForm({ ...movementForm, movementType: event.target.value as FinanceStockMovementType })} className={inputClass}>{Object.keys(MOVEMENT_LABELS).filter((item) => item !== 'TRANSFERENCIA').map((item) => <option key={item} value={item}>{MOVEMENT_LABELS[item as FinanceStockMovementType]}</option>)}</select></label><label className="text-xs font-medium">Quantidade<input required min="1" type="number" value={movementForm.quantity} onChange={(event) => setMovementForm({ ...movementForm, quantity: event.target.value })} className={inputClass} /></label><label className="text-xs font-medium">Custo unitário<input min="0" step="0.01" type="number" value={movementForm.unitCost} onChange={(event) => setMovementForm({ ...movementForm, unitCost: event.target.value })} className={inputClass} /></label><label className="text-xs font-medium">Origem<input required value={movementForm.sourceType} onChange={(event) => setMovementForm({ ...movementForm, sourceType: event.target.value })} className={inputClass} /></label><button className={buttonClass} disabled={saving}><ShoppingCart className="h-4 w-4" />Registrar movimento</button></form></div></div><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h3 className="font-semibold">Saldos atuais</h3><div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">{stockItems.map((item) => <div key={item.id} className={`rounded-xl border p-4 ${item.quantity <= item.minimumQuantity ? 'border-amber-300 bg-amber-50' : 'border-slate-200 bg-slate-50'}`}><p className="text-xs text-slate-500">{item.code}</p><p className="mt-1 font-medium">{item.name}</p><p className="mt-2 text-2xl font-semibold">{item.quantity}</p><p className="text-xs text-slate-500">mínimo {item.minimumQuantity}</p></div>)}{!stockItems.length && <p className="text-sm text-slate-500">Nenhum item de estoque cadastrado no escopo.</p>}</div></div><div className="grid gap-5 xl:grid-cols-2"><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h3 className="font-semibold">Cotações de compra</h3><form className="mt-4 grid gap-3 md:grid-cols-2" onSubmit={(event) => { event.preventDefault(); void run(async () => { await createPurchaseQuote({ unitId: quoteForm.unitId, purchaseId: quoteForm.purchaseId || undefined, supplierId: quoteForm.supplierId || undefined, status: 'ABERTA', totalAmount: Number(quoteForm.totalAmount), documentRef: quoteForm.documentRef || undefined, notes: quoteForm.notes || undefined }); }, 'Cotação registrada.'); }}><label className="text-xs font-medium">Unidade ID<input required value={quoteForm.unitId} onChange={(event) => setQuoteForm({ ...quoteForm, unitId: event.target.value })} className={inputClass} /></label><label className="text-xs font-medium">Pedido de compra ID<input value={quoteForm.purchaseId} onChange={(event) => setQuoteForm({ ...quoteForm, purchaseId: event.target.value })} className={inputClass} /></label><label className="text-xs font-medium">Fornecedor ID<input value={quoteForm.supplierId} onChange={(event) => setQuoteForm({ ...quoteForm, supplierId: event.target.value })} className={inputClass} /></label><label className="text-xs font-medium">Total<input required type="number" min="0" step="0.01" value={quoteForm.totalAmount} onChange={(event) => setQuoteForm({ ...quoteForm, totalAmount: event.target.value })} className={inputClass} /></label><button className={buttonClass} disabled={saving}><Plus className="h-4 w-4" />Registrar cotação</button></form><div className="mt-4 space-y-2">{quotes.slice(0, 8).map((quote) => <div key={quote.id} className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2 text-sm"><span>{quote.purchaseId || 'Sem pedido'} · {quote.status}</span><strong>{money(quote.totalAmount)}</strong></div>)}</div></div><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h3 className="font-semibold">Recebimento de mercadorias</h3><p className="mt-1 text-sm text-slate-500">Ao receber, o sistema cria a entrada no estoque em transação única.</p><form className="mt-4 grid gap-3 md:grid-cols-2" onSubmit={(event) => { event.preventDefault(); void run(async () => { await createGoodsReceipt({ unitId: receiptForm.unitId, purchaseId: receiptForm.purchaseId, items: [{ stockItemId: receiptForm.stockItemId, quantity: Number(receiptForm.quantity), unitCost: receiptForm.unitCost ? Number(receiptForm.unitCost) : undefined }], documentRef: receiptForm.documentRef || undefined, notes: receiptForm.notes || undefined }); }, 'Recebimento registrado e estoque atualizado.'); }}><label className="text-xs font-medium">Unidade ID<input required value={receiptForm.unitId} onChange={(event) => setReceiptForm({ ...receiptForm, unitId: event.target.value })} className={inputClass} /></label><label className="text-xs font-medium">Pedido de compra ID<input required value={receiptForm.purchaseId} onChange={(event) => setReceiptForm({ ...receiptForm, purchaseId: event.target.value })} className={inputClass} /></label><label className="text-xs font-medium">Item<select required value={receiptForm.stockItemId} onChange={(event) => setReceiptForm({ ...receiptForm, stockItemId: event.target.value })} className={inputClass}><option value="">Selecione</option>{stockItems.map((item) => <option key={item.id} value={item.id}>{item.code} — {item.name}</option>)}</select></label><label className="text-xs font-medium">Quantidade<input required min="1" type="number" value={receiptForm.quantity} onChange={(event) => setReceiptForm({ ...receiptForm, quantity: event.target.value })} className={inputClass} /></label><button className={buttonClass} disabled={saving}><Receipt className="h-4 w-4" />Registrar recebimento</button></form><div className="mt-4 space-y-2">{receipts.slice(0, 8).map((receipt) => <div key={receipt.id} className="rounded-lg bg-slate-50 px-3 py-2 text-sm">{formatDate(receipt.receivedAt)} · {receipt.status} · {receipt.items.length} item(ns)</div>)}</div></div></div><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h3 className="font-semibold">Últimos movimentos</h3><div className="mt-4 overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="border-b border-slate-200 text-xs uppercase text-slate-500"><tr><th className="px-3 py-3">Data</th><th className="px-3 py-3">Tipo</th><th className="px-3 py-3">Item</th><th className="px-3 py-3">Quantidade</th><th className="px-3 py-3">Origem</th></tr></thead><tbody className="divide-y divide-slate-100">{stockMovements.slice(0, 50).map((movement) => <tr key={movement.id}><td className="px-3 py-3">{formatDate(movement.createdAt)}</td><td className="px-3 py-3">{MOVEMENT_LABELS[movement.movementType]}</td><td className="px-3 py-3">{stockItems.find((item) => item.id === movement.stockItemId)?.name || movement.stockItemId}</td><td className="px-3 py-3">{movement.quantity}</td><td className="px-3 py-3">{movement.sourceType}</td></tr>)}</tbody></table></div></div></section>}
      </div>
    </PageShell>
  );
}
