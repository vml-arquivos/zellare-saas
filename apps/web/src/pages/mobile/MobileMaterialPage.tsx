...
const PRIORIDADES = [
  { id: 'LOW', label: 'Baixa', color: 'var(--text-secondary)' },
  { id: 'MEDIUM', label: 'Normal', color: 'var(--brand-500)' },
  { id: 'HIGH', label: 'Urgente', color: 'var(--warning)' },
];
        ...
        {CATEGORIAS.map((cat) => (
          <button type="button" key={cat.id} aria-pressed={categoria === cat.id} onClick={() => { setCategoria(cat.id); setItensSelecionados([]); }}
            style={{
              ...
              border: `0.5px solid ${categoria === cat.id ? 'var(--brand-600)' : 'var(--color-border-tertiary)'}`,
              background: categoria === cat.id ? 'var(--surface-brand)' : 'var(--color-background-primary)',
              WebkitTapHighlightColor: 'transparent',
            }}>
            <div style={{ fontSize: 28, marginBottom: 4 }}>{cat.emoji}</div>
            <p style={{ fontSize: 13, fontWeight: 500, margin: 0, color: categoria === cat.id ? 'var(--text-brand)' : 'var(--color-text-primary)' }}>{cat.label}</p>
          </button>
        ))}
      </div>
                ...
                {catInfo.itens.map((item) => (
                  <button type="button" key={item} aria-pressed={itensSelecionados.includes(item)} onClick={() => toggleItem(item)}
                    style={{
                      ...
                      border: `0.5px solid ${itensSelecionados.includes(item) ? 'var(--brand-600)' : 'var(--color-border-tertiary)'}`,
                      background: itensSelecionados.includes(item) ? 'var(--surface-brand)' : 'var(--color-background-primary)',
                      color: itensSelecionados.includes(item) ? 'var(--text-brand)' : 'var(--color-text-primary)',
              ...
              {PRIORIDADES.map((p) => (
                <button type="button" key={p.id} aria-pressed={prioridade === p.id} onClick={() => setPrioridade(p.id)}
                  style={{
                    ...
                    background: prioridade === p.id ? (p.id === 'HIGH' ? 'var(--warning-bg)' : 'var(--surface-brand)') : 'var(--color-background-primary)',
