...
const evidenceDir = "/home/ubuntu/research_zelare/onda3_journey_20260822/evidence";
...
async function assertNoHorizontalOverflow(page: Page) {
  ...
  expect(dimensions.scrollWidth, JSON.stringify(dimensions)).toBeLessThanOrEqual(dimensions.viewport);
  expect(dimensions.bodyScrollWidth, JSON.stringify(dimensions)).toBeLessThanOrEqual(dimensions.viewport);
}

test("Journey autenticada percorre abas, persiste cadastro e reflowa em 320/360/390/412/768", async ({ page }) => {
  ...
  await expect(page.getByRole("navigation", { name: "Jornada e Admissões" })).toBeVisible();
  ...
  for (const width of viewports) {
    ...
    await page.screenshot({ path: `${evidenceDir}/journey-overview-${width}.png`, fullPage: true });
  }

  await page.setViewportSize({ width: 768, height: 900 });
  await page.goto("/app/journey/interessados");
  await expect(page.getByRole("heading", { name: "Interessados" }).first()).toBeVisible();
  ...
  await expect(page.getByRole("status")).toContainText("Interessado cadastrado");
  await page.reload();
  await expect(
    page.locator("p").filter({ hasText: "Persistência Visual Sintética" }).first(),
  ).toBeVisible();
  await assertNoHorizontalOverflow(page);

  for (const [label, path] of tabs) {
    await page.getByRole("navigation", { name: "Jornada e Admissões" }).getByRole("link", { name: label, exact: true }).click();
  ...
  await page.screenshot({ path: `${evidenceDir}/journey-reports-768.png`, fullPage: true });
  writeFileSync(
